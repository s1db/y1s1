...
public class AddMonsterDialog extends JDialog implements ActionListener {
	...
	/**
	 * Creates a new AddMonsterDialog to add a monster to the given trainer, and to
	 * update the model on screen as well.
	 * 
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == cancelButton) {
			...
			// Creates and adds a Monster based on field values
			String type = (String) typeCombo.getSelectedItem();
			String attack1 = attack1Field.getText();
			int HitPoints = (Integer) hitPointSpinner.getValue();
			int attackHitPoints1 = (Integer) attack1Spinner.getValue();
			String attack2 = attack2Field.getText();
			int attackHitPoints2 = (Integer) attack2Spinner.getValue();
			Monster newMonster = null;
			Attack newAttack1 = null;
			Attack newAttack2 = null;
			Attack[] newAttacks = new Attack[2];

			try {
				newAttack1 = new Attack(attack1, attackHitPoints1);
				newAttack2 = new Attack(attack2, attackHitPoints2);
				newAttacks = new Attack[] { newAttack1, newAttack2 };
			} catch (Exception ex) {
				// Don't create attacks and monster
				dispose();
			}
			try {
				switch (type) {
				case "Water":
					newMonster = new WaterMonster(HitPoints, newAttacks);
					break;

				case "Fire":
					newMonster = new FireMonster(HitPoints, newAttacks);
					break;
				case "Electric":
					newMonster = new FireMonster(HitPoints, newAttacks);
					break;
				...
				// Don't create monster
				dispose();
			}

			// If we have created a new monster with valid attacks, then add it to the list
			// in the main window
			if ((newMonster != null) && (newAttacks != null)) {
				model.addElement(newMonster);
				trainer.addMonster(newMonster);
			}
